...
def run_model(nsteps, ncells, dumpfreq, Omega, dt):
    # =======================================================================
    ...
    rho0 = 1                     # constant density of boundary layer
                                 # air (kg m^-3)
    cH = 1e-3                    # bulk coefficient (no units)
    L = 2.5e6                    # latent heat of condensation (J kg^-1)
    Rw = 416                     # specific gas constant for water
                                 # vapour (J K^-1 kg^-1)
    e0 = 2300*exp(L/(Rw*293.))   # reference saturation vapour pressure (Pa)
    p0 = 1e5                     # reference pressure (Pa)
    mB = 0.006029                # boundary layer overturning
                                 # timescale (kg m^-2 s^-1)

    # =======================================================================
    # Parameters used to compute vertical velocity w
    Cp = 1005.     # specific heat capacity at constant pressure of
                   # dry air (J K^-1 kg^-1)
    dtheta = 50    # change in potential temperature across troposphere (K)
    Qcl = 100.     # net combined radiative-sensible cooling of free
                   # atmosphere (W m^-2)
    qC = 0.018     # critical specific humidity for initiation of
                   # convection (kg kg^-1)
    ...
    t = 0
    tmax = nsteps * dt
    not_steady = True      # flag to indicate that we have not yet
                           # reached a steady state
    not_blowing_up = True  # flag to indicate that we have not yet blown up
    first_step = True      # we need to know if we're at the first
                           # step so that we can compute an initial
                           # change in h to determine later whather we
                           # are blowing up
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--nsteps',
        help="The maximum number of timesteps to run the simulation.",
        type=int,
        default=100
    )
    ...
    parser.add_argument(
        '--dumpfreq',
        help="The frequency at which to produce plots (number of timesteps).",
        type=int,
        default=10
    )
    ...
    parser.add_argument(
        '--Omega',
        help="The rotation rate of planet (rad s^-1).",
        type=float,
        default=6.501e-6
    )
